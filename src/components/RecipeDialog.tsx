...
export default function RecipeDialog() {
  ...
  if (!recipe) {
    return (
      <Dialog fullScreen open onClose={handleClose} slots={{ transition: Transition }}>
        ...
        <Container maxWidth="md" sx={{ py: 4 }}>
          ...
          <Stack direction="row" spacing={0.75} flexWrap="wrap" useFlexGap sx={{ mb: 3 }}>
            {flattenKeywords(recipe.keywords).map((kw) => (
              <Chip key={kw} label={kw} size="small" sx={{ bgcolor: "secondary.light", color: "text.secondary" }} />
            ))}
          </Stack>
          ...
          <Stack direction="row" spacing={1.5} sx={{ mb: 4 }} flexWrap="wrap" useFlexGap>
            <Chip
              ...
              sx={{ bgcolor: "primary.light", color: "#fff", fontWeight: 500 }}
            />
            <Chip
              icon={<LocalFireDepartmentIcon />}
              label={`Cook: ${recipe.cookTime} min`}
              sx={{ bgcolor: "secondary.main", color: "#fff", fontWeight: 500 }}
            />
            <Chip
              icon={<TimerIcon />}
              label={`Total: ${recipe.totalTime} min`}
              variant="outlined"
              sx={{ borderColor: "#5b9bd5", borderWidth: 2, color: "#5b9bd5", fontWeight: 500 }}
            />
            <Chip
              icon={<PeopleIcon />}
